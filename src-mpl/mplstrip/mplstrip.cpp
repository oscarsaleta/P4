...
void PrintHelp(void)
{
    printf("\n"
           ...
           "    -head       Inserts an automatically generated heading in the "
           "maple file.\n"
...
int ProcessOption(char *opt, char *extraarg)
{
    ...
    if (!strcmp(opt, "M") || !strncmp(opt, "M=", 2)) {
        if (opt[1] == '=') {
            opt += 2;
            while (isspace(*opt) && *opt != 0)
                opt++; // remove white space
            if (*opt != 0) {
...
void RunMaple(char *fname)
{
    ...
    if (strchr(MapleCommand, ' '))
        strcat(
            strcat(strcat(strcat(strcpy(cmd, "\""), MapleCommand), "\""), " "),
            fname);
    else
...
void TranslateFile(const char *src, const char *dst)
{
    ...
    if (!Opt_Quiet)
        printf("A number of %d lines have been extracted from a total of %d "
               "lines.\n",
               writtenlines, linenr);
