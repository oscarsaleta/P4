                                           ...
                                           int dir, int type, int points_to_int, struct orbits_points **orbit )
{
    ...
    */
    ...
    MATHFUNC(integrate_sphere_sep)(pcoord[0],pcoord[1],pcoord[2],pcoord,
                       &hhi,&type,&color,&dashes,&d,h_min,h_max);     
    ...
    {last_orbit->next_point=(struct orbits_points *) malloc(sizeof
...
struct orbits_points
  * plot_separatrice(QWinSphere * spherewnd, double x0, double y0,double a11, double a12, double a21, double a22,
  double epsilon, struct sep * sep1, struct orbits_points ** orbit, short int chart)
{
...
/* h=(epsilon/100)*sep1->direction; */
 ...
 for(i=0;i<=99;i++){
  ...
  if((t*t+y*y)>epsilon*epsilon)break; 
 
  if(sep1->d){point[0]=x0+a11*y+a12*t;point[1]=y0+a21*y+a22*t;}
   else {point[0]=x0+a11*t+a12*y;point[1]=y0+a21*t+a22*y;}
    last_orbit->next_point=(struct orbits_points *) malloc(sizeof            
   (struct orbits_points));
  last_orbit=last_orbit->next_point;
  dashes=true; 
  switch(chart){
    case CHART_R2: MATHFUNC(R2_to_sphere)(point[0],point[1],pcoord);
            color=FindSepColor(VFResults.gcf,sep1->type,point); 
            break;
    case CHART_U1: if(point[1]>=0 || !VFResults.singinf){MATHFUNC(U1_to_sphere)(point[0],point[1],pcoord);
                                          if(!ok){dashes=false; ok=true;
                                                  if(VFResults.dir_vec_field==1)dir*=-1;}
                                          type=sep1->type;
                                          color=FindSepColor(VFResults.gcf_U1,type,point);                                            
                                         }
             else{VV1_to_psphere(point[0],point[1],pcoord);
                  if(ok){dashes=false; ok=false;
                         if(VFResults.dir_vec_field==1)dir*=-1;}
                  psphere_to_V1(pcoord[0],pcoord[1],pcoord[2],point);
                  if(VFResults.dir_vec_field==1)type=change_type(sep1->type); 
                  else type=sep1->type;
                  color=FindSepColor(VFResults.gcf_V1,type,point);
                 } 
            break;
    case CHART_V1: MATHFUNC(V1_to_sphere)(point[0],point[1],pcoord);
            if((VFResults.p==1) && (VFResults.q==1))psphere_to_V1(pcoord[0],pcoord[1],pcoord[2],point);
            color=FindSepColor(VFResults.gcf_V1,sep1->type,point); 
            break;
    case CHART_U2:if(point[1]>=0 || !VFResults.singinf){MATHFUNC(U2_to_sphere)(point[0],point[1],pcoord);
                                         if(!ok){dashes=false; ok=true;
                                                  if(VFResults.dir_vec_field==1)dir*=-1;}
                                          type=sep1->type;
                                          color=FindSepColor(VFResults.gcf_U2,type,point);
                                         }
            else{VV2_to_psphere(point[0],point[1],pcoord);
                 if(ok){dashes=false; ok=false;
                         if(VFResults.dir_vec_field==1)dir*=-1;}
            ...
            break;
    case CHART_V2: MATHFUNC(V2_to_sphere)(point[0],point[1],pcoord);
            if((VFResults.p==1) && (VFResults.q==1))psphere_to_V2(pcoord[0],pcoord[1],pcoord[2],point);
...
{
 if(b==0)return(1.); else return(pow(a,b));
}
...
/*   h=find_step(de_sep->sep,epsilon,1.0)/100;  */
  ...
  for(i=0;i<=99;i++){
   ...
   make_transformations(de_sep->trans,
    de_sep->x0+de_sep->a11*t+de_sep->a12*y,
   ...
   switch(chart){
    ...
    case CHART_U1: if(point[1]>=0 || !VFResults.singinf){MATHFUNC(U1_to_sphere)(point[0],point[1],pcoord);
                 ...
                 if(ok){dashes=false; ok=false;}
                 psphere_to_V1(pcoord[0],pcoord[1],pcoord[2],point);
                 if(VFResults.dir_vec_field==1)type=change_type(de_sep->type); 
                 else type=de_sep->type;
                 color=FindSepColor(VFResults.gcf_V1,type,point);     
                 } 
            break;
    case CHART_V1: MATHFUNC(V1_to_sphere)(point[0],point[1],pcoord);
  ...
  de_sep->point[0]=t; de_sep->point[1]=y;
...
void start_plot_saddle_sep( QWinSphere * spherewnd )
{
 ...
 {VFResults.selected_sep->first_sep_point=plot_separatrice(spherewnd, VFResults.selected_saddle_point->x0,
    VFResults.selected_saddle_point->y0,VFResults.selected_saddle_point->a11,VFResults.selected_saddle_point->a12,
    VFResults.selected_saddle_point->a21,VFResults.selected_saddle_point->a22,VFResults.selected_saddle_point->epsilon,
    VFResults.selected_sep,&points,VFResults.selected_saddle_point->chart);
  VFResults.selected_sep->last_sep_point=points;
 }
}
...
void start_plot_se_sep( QWinSphere * spherewnd )
{
  ...
  if(VFResults.selected_sep->last_sep_point)
  {copy_x_into_y(VFResults.selected_sep->last_sep_point->pcoord,p);
   VFResults.selected_sep->last_sep_point->next_point=integrate_sep(spherewnd,p,VFResults.config_currentstep,
  ...
  { 
   VFResults.selected_sep->first_sep_point=plot_separatrice(spherewnd,VFResults.selected_se_point->x0,
     VFResults.selected_se_point->y0,VFResults.selected_se_point->a11,VFResults.selected_se_point->a12,
     VFResults.selected_se_point->a21,VFResults.selected_se_point->a22,VFResults.selected_se_point->epsilon,
     VFResults.selected_sep,&points,VFResults.selected_se_point->chart);   
     VFResults.selected_sep->last_sep_point=points;
  }
}
