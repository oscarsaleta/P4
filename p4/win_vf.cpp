...
//  setFont( QFont( FONTSTYLE, FONTSIZE ) );
    ...
    if( ThisVF->numparams != 0 )
    {
        ...
        {   
            ...
            QObject::connect( sb_params, SIGNAL(valueChanged(int)), params, SLOT(paramsSliderChanged(int)) );
            QObject::connect( sb_params, SIGNAL(sliderMoved(int)), params, SLOT(paramsSliderChanged(int)) );
        }
        params->show();
        if( ThisVF->numparams > MAXNUMPARAMSSHOWN )
            sb_params->show();
    ...
    QObject::connect( spin_numparams, SIGNAL(valueChanged(int)), this, SLOT(numParamsChanged(int)) );
...
void QVectorFieldDlg::GetDataFromDlg( void )
{
    ...
    if( xdot.compare( ThisVF->xdot ) ||
        ydot.compare( ThisVF->ydot ) ||
...
QVFParams::QVFParams( QVectorFieldDlg * parent, QScrollBar * sb )
    : QWidget( parent )
{
    int i;
    ...
    currentshownparams = (currentnumparams < MAXNUMPARAMSSHOWN ) ? currentnumparams : MAXNUMPARAMSSHOWN;
    currentpageindex = 0;
    ...
    for( i = 0; i < currentshownparams; i++ )
    {
        ...
        Param_Values[i]->setToolTip( "Enter the value of this parameter during this evaluation session" );
#endif
    }
...
bool QVFParams::focusNextPrevChild( bool next )
{
    ...
    if( p == Param_Values[currentshownparams-1] && currentpageindex < currentnumparams - MAXNUMPARAMSSHOWN && next )
    {
        sb_params->setValue( currentpageindex+1 );
...
bool QVFParams::UpdateDlgData( void )
{
    ...
    if( ThisVF->numparams != currentnumparams )
    {
        ...
        // set a flag to say that the current data in this object is no longer valid.
    }
