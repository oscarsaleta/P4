...
#define DEFAULT_SYMBOLSIZE  1.6         // default symbol size (mm) when printing the singularities
#define MIN_SYMBOLSIZE      0.05            // minimum symbol size
#define MAX_SYMBOLSIZE      30.0            // maximum symbol size

#define DEFAULT_LINEWIDTH   0.3         // default line width (mm) when printing orbits etc
...
// uncomment resp. comment following lines to enable resp. disable docking feature:
...
// IN PRINT_BITMAP.CPP: PrintColorTable shows how these colors are treated when printing.
// For example, when printing, BLACK can be reversed with WHITE, so that the BLACK
// background on screen is not printed black.

// comment following line if you want an EXACT screen copy (with black background)
...
// if z coord on the poincare sphere is less than ZCOORD then point goes to infinity:
...
// when producing an (E)PS image, P4 will try to center the 15-cm wide image on a white
// page.  Therefore it needs to know the dimensions of the output paper.  By default,
// it will assume European A4 format.  The following dimensions are given in inches:
