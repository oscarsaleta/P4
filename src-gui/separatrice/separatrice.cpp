 ...
 *
 *  Copyright (C) 1996-2017  J.C. Artés, C. Herssens, P. De Maesschalck,
...
// #define LOGACTIVITY      // define when you want to create a log file of all
// tests (sep.log)
                        ...
                        double *diff_slope, double *dist)
{
    ...
    for (i = 0; i < NUMBER; ++i) {
        /*
        printf("i=%d vec_slope=%15.13e sep_slope=%15.13e diff_slope=%15.13e
        a=%10.9e\n",
        ...
        do {
            if (current_term2->exp_x && current_term2->exp_y)
                s += current_term2->coeff *
                     pow(a, (double)current_term2->exp_x) *
                     pow(b, (double)current_term2->exp_y);
            else if (current_term2->exp_x)
                s +=
                    current_term2->coeff * pow(a, (double)current_term2->exp_x);
            else if (current_term2->exp_y)
                s +=
                    current_term2->coeff * pow(b, (double)current_term2->exp_y);
            else
                s += current_term2->coeff;
...
void read_term2(FILE *fp, struct term2 *current_term2, int l)
{
    int i;
    if (fscanf(fp, "%i %i %lf \n\n", &current_term2->exp_x,
               &current_term2->exp_y, &current_term2->coeff) != 3)
        exit(1);
    current_term2->next_term2 = nullptr;
    for (i = 2; i <= l; i++) {
        current_term2->next_term2 =
            new term2; //(struct term2 *) malloc (sizeof(struct term2));
...
void read_term1(FILE *fp, struct term1 *current_term1, int l)
{
    ...
    for (i = 2; i <= l; i++) {
        current_term1->next_term1 =
            new term1; //(struct term1 *) malloc (sizeof(struct term1));
        current_term1 = current_term1->next_term1;
        if (fscanf(fp, "%i %lf \n\n", &current_term1->exp,
                   &current_term1->coeff) != 2)
            exit(1);
        current_term1->next_term1 = nullptr;
    }
}
