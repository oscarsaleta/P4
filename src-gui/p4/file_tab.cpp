...
bool QVFStudy::readCurve(QString basename)
{
    ...
    curves new_curve;
    if (fscanf(fp, "%d", &degree_curve) != 1) return false;
    ...
    if (degree_curve > 0) {
        ...
        if (!readTerm2(fp, new_curve.r2, N)) return false;
        ...
        if (!readTerm2(fp, new_curve.u1, N)) return false;
        ...
        if (!readTerm2(fp, new_curve.u2, N)) return false;

        if (fscanf(fp, "%d", &N) != 1) return false;

        new_curve.v1 = new term2;
        new_curve.v1->next_term2 = nullptr;
        if (!readTerm2(fp, new_curve.v1, N)) return false;

        if (fscanf(fp, "%d", &N) != 1) return false;
        new_curve.v2 = new term2;
        new_curve.v2->next_term2 = nullptr;
        if (!readTerm2(fp, new_curve.v2, N)) return false;
        ...
        if (p_ != 1 || q_ != 1) {
            ...
            if (!readTerm3(fp, new_curve.c, N)) return false;
        }
    } else {
        return false;
    }
...
bool QVFStudy::readIsoclines(QString basename)
{
    ...
    if (degree_curve > 0) {
        ...
        if (!readTerm2(fp, new_isocline.r2, N)) return false;
        ...
        if (!readTerm2(fp, new_isocline.u1, N)) return false;
        ...
        if (!readTerm2(fp, new_isocline.u2, N)) return false;

        if (fscanf(fp, "%d", &N) != 1) return false;

        new_isocline.v1 = new term2;
        new_isocline.v1->next_term2 = nullptr;
        if (!readTerm2(fp, new_isocline.v1, N)) return false;

        if (fscanf(fp, "%d", &N) != 1) return false;
        new_isocline.v2 = new term2;
        new_isocline.v2->next_term2 = nullptr;
        if (!readTerm2(fp, new_isocline.v2, N)) return false;
        ...
        if (p_ != 1 || q_ != 1) {
            ...
            if (!readTerm3(fp, new_isocline.c, N)) return false;
        } else {
...
bool QVFStudy::readSemiElementaryPoint(FILE *fp)
{
    ...
    switch (point->type) {
        ...
        default:
            ...
            switch (point->chart) {
                case CHART_R2:
                    if (eval_term2(gcf_, y) < 0) ok = true;
                    break;
                case CHART_U1:
                    if (eval_term2(gcf_U1_, y) < 0) ok = true;
                    break;
                case CHART_V1:
                    if ((p_ == 1) && (q_ == 1)) y[0] = -y[0];
                    if (eval_term2(gcf_V1_, y) < 0) ok = true;
                    break;
                case CHART_U2:
                    if (eval_term2(gcf_U2_, y) < 0) ok = true;
                    break;
                case CHART_V2:
                    if ((p_ == 1) && (q_ == 1)) y[0] = -y[0];
                    if (eval_term2(gcf_V2_, y) < 0) ok = true;
                    break;
            }
            if (ok) {
                if (point->type == 5)
...
bool QVFStudy::readStrongFocusPoint(FILE *fp)
{
    ...
    switch (point->chart) {
        case CHART_R2:
            if (eval_term2(gcf_, y) < 0) point->stable *= -1;
            break;

        case CHART_U1:
            if (eval_term2(gcf_U1_, y) < 0) point->stable *= -1;
            break;

        case CHART_V1:
            if (p_ == 1 && q_ == 1) y[0] = -y[0];

            if (eval_term2(gcf_V1_, y) < 0) point->stable *= -1;
            break;

        case CHART_U2:
            if (eval_term2(gcf_U2_, y) < 0) point->stable *= -1;
            break;

        case CHART_V2:
            if (p_ == 1 && q_ == 1) y[0] = -y[0];

            if (eval_term2(gcf_V2_, y) < 0) point->stable *= -1;
            break;
    }
...
bool QVFStudy::readWeakFocusPoint(FILE *fp)
{
    ...
    if ((point->type == STYPE_STABLE) || (point->type == STYPE_UNSTABLE)) {
        ...
        switch (point->chart) {
            case CHART_R2:
                if (eval_term2(gcf_, y) < 0) point->type *= -1;
                break;

            case CHART_U1:
                if (eval_term2(gcf_U1_, y) < 0) point->type *= -1;
                break;

            case CHART_V1:
                if ((p_ == 1) && (q_ == 1)) y[0] = -y[0];
                if (eval_term2(gcf_V1_, y) < 0) point->type *= -1;
                break;

            case CHART_U2:
                if (eval_term2(gcf_U2_, y) < 0) point->type *= -1;
                break;

            case CHART_V2:
                if ((p_ == 1) && (q_ == 1)) y[0] = -y[0];
                if (eval_term2(gcf_V2_, y) < 0) point->type *= -1;
                break;
        }
    }
