...
orbits_points *g_last_isoclines_point[2] = {nullptr, nullptr};
...
// function definitions FIXME: segfault a draw_isoclines
bool evalIsoclinesStart(QWinSphere *sp, int dashes, int precision, int points)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        result[i] = runTaskIsoclines(i, s_IsoclinesTask, precision, points);
    }
...
bool evalIsoclinesFinish(void) // return false in case an error occured
{
    for (int i = 0; i < 2; i++) {
        if (s_IsoclinesTask != EVAL_ISOCLINES_NONE) {
            s_IsoclinesSphere->prepareDrawing();
            draw_isoclines(s_IsoclinesSphere,
                           g_VFResults.current_isoclines_->points[i], CCURV, 1);
...
bool runTaskIsoclines(int i, int task, int precision, int points)
{
    ...
    case EVAL_ISOCLINES_R2:
        value = g_ThisVF->prepareIsoclines(
            g_VFResults.current_isoclines_->r2[i], -1, 1, precision, points);
        break;
    case EVAL_ISOCLINES_U1:
        value = g_ThisVF->prepareIsoclines(
            g_VFResults.current_isoclines_->u1[i], 0, 1, precision, points);
        break;
    case EVAL_ISOCLINES_V1:
        value = g_ThisVF->prepareIsoclines(
            g_VFResults.current_isoclines_->u1[i], -1, 0, precision, points);
        break;
    case EVAL_ISOCLINES_U2:
        value = g_ThisVF->prepareIsoclines(
            g_VFResults.current_isoclines_->u2[i], 0, 1, precision, points);
        break;
    case EVAL_ISOCLINES_V2:
        value = g_ThisVF->prepareIsoclines(
            g_VFResults.current_isoclines_->u2[i], -1, 0, precision, points);
        break;
...
{
    if (g_VFResults.current_isoclines_->points[i] != nullptr) {
        g_last_isoclines_point[i]->next_point = new orbits_points;
        g_last_isoclines_point[i] = g_last_isoclines_point[i]->next_point;
    } else {
        g_last_isoclines_point[i] = new orbits_points;
        g_VFResults.current_isoclines_->points[i] = g_last_isoclines_point[i];
    }

    g_last_isoclines_point[i]->pcoord[0] = x0;
    g_last_isoclines_point[i]->pcoord[1] = y0;
    g_last_isoclines_point[i]->pcoord[2] = z0;

    g_last_isoclines_point[i]->dashes = dashes;
    g_last_isoclines_point[i]->color = CCURV;
    g_last_isoclines_point[i]->next_point = nullptr;
}
