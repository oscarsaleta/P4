    ...
    if (point) {
        do {
            ...
            if ((d < distance && !std::isnan(d) && p4_finite(d)) ||
                (distance == -1)) {
                distance = d;
                VFResults.selected_saddle_point_ = point;
                VFResults.selected_ucoord_[0] = ucoord[0];
                VFResults.selected_ucoord_[1] = ucoord[1];
                *type = SADDLE;
            }
        ...
        do {
            if (point->separatrices) {
                ...
                if ((d < distance && !std::isnan(d) && p4_finite(d)) ||
                    (distance == -1)) {
                    distance = d;
                    VFResults.selected_se_point_ = point;
                    VFResults.selected_ucoord_[0] = ucoord[0];
                    VFResults.selected_ucoord_[1] = ucoord[1];
            ...
            if (point->blow_up) {
                ...
                if ((d < distance && !std::isnan(d) && p4_finite(d)) ||
                    ...
                    if ((d < distance) || (distance == -1)) {
                        ...
                        VFResults.selected_ucoord_[0] = ucoord[0];
                        VFResults.selected_ucoord_[1] = ucoord[1];
                        VFResults.selected_de_point_ = point;
                        *type = NON_ELEMENTARY;
                    }
            }
...
int find_critical_point(QWinSphere *spherewnd, double x, double y)
{
    ...
    switch (type) {
    case SADDLE:
        sepc = VFResults.selected_sep_ =
            VFResults.selected_saddle_point_->separatrices;
        ...
        s = "SADDLE";
        epsilon = VFResults.selected_saddle_point_->epsilon;
        change_epsilon = change_epsilon_saddle;

        if (VFResults.selected_saddle_point_->chart == CHART_R2) {
            sx.sprintf("x = %f", (float)(VFResults.selected_saddle_point_->x0));
            sy.sprintf("y = %f", (float)(VFResults.selected_saddle_point_->y0));
            sz = "";
        } else {
            switch (VFResults.selected_saddle_point_->chart) {
            case CHART_U1:
                MATHFUNC(U1_to_sphere)
                (VFResults.selected_saddle_point_->x0, 0.0, pcoord);
                break;
            case CHART_V1:
    ...
    case SEMI_HYPERBOLIC:
        sepc = VFResults.selected_sep_ =
            VFResults.selected_se_point_->separatrices;
        draw_selected_sep(spherewnd, VFResults.selected_sep_->first_sep_point,
                          CW_SEP);
        ...
        s = "SEMI-HYPERBOLIC";
        epsilon = VFResults.selected_se_point_->epsilon;
        change_epsilon = change_epsilon_se;
        if (VFResults.selected_se_point_->chart == CHART_R2) {
            sx.sprintf("x=%f", (float)(VFResults.selected_se_point_->x0));
            sy.sprintf("y=%f", (float)(VFResults.selected_se_point_->y0));
            sz = "";
        } else {
            switch (VFResults.selected_se_point_->chart) {
            case CHART_U1:
                MATHFUNC(U1_to_sphere)
                (VFResults.selected_se_point_->x0, 0.0, pcoord);
                break;
            case CHART_V1:
                MATHFUNC(V1_to_sphere)
                (VFResults.selected_se_point_->x0, 0.0, pcoord);
    ...
    case NON_ELEMENTARY:
        ...
        for (bc = VFResults.selected_de_sep_; bc != nullptr;
        ...
        s = "NON-ELEMENTARY";
        epsilon = VFResults.selected_de_point_->epsilon;
        change_epsilon = change_epsilon_de;
        if (VFResults.selected_de_point_->chart == CHART_R2) {
            sx.sprintf("x=%f", (float)(VFResults.selected_de_point_->x0));
            sy.sprintf("y=%f", (float)(VFResults.selected_de_point_->y0));
            sz = "";
        } else {
            switch (VFResults.selected_de_point_->chart) {
            case CHART_U1:
                MATHFUNC(U1_to_sphere)
                (VFResults.selected_de_point_->x0, 0.0, pcoord);
                break;
            case CHART_V1:
                MATHFUNC(V1_to_sphere)
                (VFResults.selected_de_point_->x0, 0.0, pcoord);
